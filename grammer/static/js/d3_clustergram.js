...
function make_d3_clustergram(network_data) {
  ...
  // clust_group = d3.select('#svg_div_exp')
  //!! making separate clustergrams 
  clust_group = d3.select('#'+select_svg_div)
      .append("svg")
      .attr('id', 'main_svg')
  ...
  row_label_obj
    .each(function(){
      ...
      .style('fill',function(d){
        var inst_hl = 'yellow';
        // // highlight kinases in red 
        // if (ccle_params['gc'] == 'ALL'){
        //   if (d.gc == 'KIN'){
        //     inst_hl = 'red';
        //   }
        // };
      ...
      .style('opacity',function(d){
        var inst_opacity = 0;
        // // highlight transcription factors if viewing all genes
        // if (ccle_params['gc'] == 'ALL'){
          // highlight genes 
          ...
          // if (d.gc == 'TF'){
          //   inst_opacity = 1;
          //   console.log('found transcription factors')
          // };
...
function reorder_clust_rank() {

  // load orders 
  if ( ccle_params['order'] == 'clust' ){ 
    // order by enrichment 
    x_scale.domain(orders.clust_row);
    y_scale.domain(orders.clust_col);
  }
  else if ( ccle_params['order'] == 'rank'){
    // order by enrichment 
...
function initialize_clustergram(network_data){
  ...
  // set max zscore opacity to arbitrary value of 4 zscore 
  opacity_scale = d3.scale.linear().domain([0, Math.abs(max_link.value) ]).clamp(true).range([0.0,1.0]) ; 
  // opacity_scale = d3.scale.linear().domain([0, 6 ]).clamp(true).range([0.0,1.0]) ; 
...
function set_visualization_size(){
  ...
  if (col_nodes.length > row_nodes.length){
    ...
    if (viz_height > svg_height){
      //!! I need to improve the rules here, this is causing the rects to be narrow
  ...
  // // no longer initiate in clustered state 
  // x_scale.domain(orders.clust_row);
  // y_scale.domain(orders.clust_col);
